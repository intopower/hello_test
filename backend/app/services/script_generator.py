...
class OpenAIScriptGenerationService:
    ...
    def generate(
        ...
        *,
        ...
        lines = list(transcript) or ["剧情即将展开，敬请期待"]
            ...
            input=[
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": "你是一名中文短视频解说脚本编剧，需要把电视剧对白转成 3-5 段有节奏的口播稿，并给出对应情绪与关键词。",
                        }
                    ],
                },
                {
        ...
        return RuleBasedScriptGenerationService.THE_FALLBACK.generate(
            lines, target_duration=target_duration, language=language
        )
...
class RuleBasedScriptGenerationService:
    THEMES = [
        ("dreamy_cinematic", "空灵电影感"),
        ("dark_suspense", "暗黑悬疑"),
        ("retro_romance", "复古情怀"),
    ]
    THE_FALLBACK: "RuleBasedScriptGenerationService"  # singleton helper
        ...
        *,
        ...
        for idx, line in enumerate(lines, start=1):
            ...
            segments.append(
                ScriptSegment(
                    order=idx,
                    text=f"[{language.upper()}] {line}（自动规则生成）",
                    start=start,
                    end=end,
...
RuleBasedScriptGenerationService.THE_FALLBACK = RuleBasedScriptGenerationService()
